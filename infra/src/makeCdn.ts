...
export function makeCdn(scope: Construct, apiUrl: string) {
  const frontendBucket = new Bucket(scope, "botsFrontendBucket", {
  ...
  const distribution = new Distribution(scope, "botsDist", {
    additionalBehaviors: {
      "*.html": {
        ...behaviorOptions,
  ...
  new BucketDeployment(scope, "botsFrontendDeploy", {
